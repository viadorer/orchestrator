...
import { Pencil, ImageIcon, Check, X, Trash2, Send, ChevronLeft, ChevronRight, Save, TrendingUp, Sparkles, Clock, Loader2 } from 'lucide-react';
...
export function PostCard({
  ...
  return (
    ...
    >
      ...
      {/* Inline scheduler (appears after clicking clock icon) */}
      {showScheduler && onApproveAndSchedule && (
        <div className="px-3 pb-2">
          <div className="p-2.5 rounded-lg bg-slate-800 border border-violet-500/30 space-y-2">
            <div className="flex items-center gap-2">
              <input
                type="datetime-local"
                value={scheduleDate}
                onChange={(e) => setScheduleDate(e.target.value)}
                className="flex-1 px-2 py-1.5 rounded-lg bg-slate-900 border border-slate-700 text-white text-xs focus:outline-none focus:ring-2 focus:ring-violet-500 min-w-0"
              />
            </div>
            <div className="flex items-center gap-2">
              <button
              ...
              >
                {scheduling ? (
                  ...
                  <><Clock className="w-3 h-3" /> Schválit & Naplánovat</>
                ) : (
                  <><Send className="w-3 h-3" /> Schválit & Odeslat</>
                )}
              </button>
              <button
                onClick={() => { setShowScheduler(false); setScheduleDate(''); }}
